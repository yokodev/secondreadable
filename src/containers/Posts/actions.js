...
export const commentsForThisPost = (id)=>
dispatch=>{
  dispatch({type:GETTING_COMMENTS_FOR_THIS_POST})
  return API.allCommentsByPostId(id)
  .then(
    comments => dispatch({type:GETTING_COMMENTS_FOR_THIS_POST_SUCCESS, id, comments}),
    error=> {dispatch({type:GETTING_COMMENTS_FOR_THIS_POST_FAILURE, error}); throw error}
  )
}

export const allPostsWComments = ()=>
(dispatch,getState)=>{
  return dispatch(getPosts())
  .then(()=>{
    const allIds=getState().posts.allIds
    allIds && allIds.length>0
    && allIds.map(singlePostId=>
      dispatch(commentsForThisPost(singlePostId)))
  })
}
export const allPostsWCommentsNCat = (cat)=>
(dispatch,getState)=>{
  return dispatch(getPostsByCat(cat))
  .then(()=>{
    const allIds=getState().posts.allIds
    allIds && allIds.length>0
    && allIds.map(singlePostId=>
  ...
  return function(dispatch){
    return API.getPosts()
      .then(
        ...
        error=>{dispatch({type:RECEIVE_POSTS_FAILURE, error}); throw error}
      )
  }
}
