...
export const commentsByPost = postId => dispatch => {
  ...
  return API.allCommentsByPostId(postId).then(
    response => {
      dispatch(commentsByIdSuccess(response))
      dispatch({
        type: SET_COMMENT_TO_DETAIL,
...
/***********SET-COMMENT-RATE************/
export const setCommentRate = (commentId, voteScore) => dispatch => {
  dispatch({ type: SET_COMMENT_RATE })
  return API.voteOnComment(commentId, voteScore).then(
    response => dispatch({ type: SET_COMMENT_RATE_SUCCESS, response }),
    error => {
...
/***********ADD-NEW-COMMENT************/
export const addNewComment = (postId, formValues) => dispatch => {
  dispatch({ type: ADD_COMMENT })
  return API.addNewComment(postId, formValues).then(
    response => {
...
export const editComment = (commentId, body) => dispatch => {
  ...
  return API.editComment(commentId, body).then(
    response => {
...
export const getThisComment = (commentId, callback) => dispatch => {
  dispatch({ type: GET_COMMENT_DETAIL })
  return API.getCommentDetail(commentId).then(
    response => {
